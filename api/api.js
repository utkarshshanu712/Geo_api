...
// Function to find state based on coordinates
function findState(lat, long) {
  let foundState = "Unknown State";
  let foundDistrict = "";
  let isInIndia = false;

  for (const [state, coordinates] of Object.entries(indianStatesCoordinates)) {
    if (
      Math.abs(coordinates.latitude - lat) <= 0.5 &&
      Math.abs(coordinates.longitude - long) <= 0.5
    ) {
      foundState = state.replace(/([A-Z])/g, " $1").trim();
      isInIndia = true;
  ...
  return foundDistrict
    ? `${foundState}, ${foundDistrict} India`
    : `${foundState} India`;
}

// API endpoint to get state and district based on coordinates
app.get("/find-state", (req, res) => {
  ...
  const result = findState(lat, long);
